...
RegisterNetEvent("bir-uiitem:SendNotify")
AddEventHandler("bir-uiitem:SendNotify", function(type, item, options)
	...
	for i=1, #currentAccounts, 1 do
		...
		if  currentAccounts[i].name == 'black_money' and not isDropBlackMoney	 then
			if currentAccounts[i].money > account.money then
				...
				TriggerEvent("bir-uiitem:SendNotify", 'remove',  {
					name = 'black_money	',
				...
				TriggerEvent("bir-uiitem:SendNotify", 'add',  {
					name = 'black_money	',
					label = (Config.BlackMoneyLabel):format(ESX.Math.GroupDigits(ESX.Math.Round(money)))
				})
			end
...
AddEventHandler('_esx:useItem', function(item)
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("bir-uiitem:SendNotify", 'use', item)
	end
end)
...
AddEventHandler('_esx:dropItem', function(item)
	...
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("bir-uiitem:SendNotify", 'remove', item, {action = 'ทิ้ง'})
	end
end)
...
AddEventHandler('esx:addInventoryItem', function(item, count)
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("bir-uiitem:SendNotify", 'add', item)
	end
end)
...
AddEventHandler('_esx:removeInventoryItem', function(item, count)
	...
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("bir-uiitem:SendNotify", 'remove', item)
	end
end)
